...
//   // Calculate the total items in the cart using reduce
//   const totalItemsInCart = cart?.reduce((total: number, item: { quantity: number }) => total + item.quantity, 0) || 0;
...
import Link from "next/link";
import Image from "next/image";
import { useCart } from "@/components/cartcomponent/cart"; // Import the useCart hook and CartItem type

export default function Navbar() {
  const [isMobileMenuOpen, setMobileMenuOpen] = useState(false);
  const { cartItems } = useCart(); // Access cartItems from context

  // Calculate the total items in the cart (by counting how many instances of each item exist)
  ...
  return (
    <header className="bg-white border-b shadow-sm">
      ...
      <div className="container mx-auto flex items-center justify-between py-4 px-4">
        ...
        <nav className="hidden md:flex">
          <ul className="flex space-x-8">
            {["Home", "Shop", "Product", "Pages", "About", "Contact"].map((item) => (
              <li key={item}>
                <Link
